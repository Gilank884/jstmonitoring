...
export default function WorkOrderDetail() {
    ...
    const loadPhotosAndSignatures = async (orderData) => {
        try {
            // Determine the actual storage prefix used for this order's files.
            // Some uploads historically added an extra `workorder/` prefix to the path.
            let prefixCandidate1 = `${orderData.no_spk}`; // e.g. "012008"
            let prefixCandidate2 = `workorder/${orderData.no_spk}`; // e.g. "workorder/012008"
            ...
            try {
                ...
                if (listErr1) {
                    console.debug("list prefix1 error:", listErr1);
                }
                if (Array.isArray(list1) && list1.length > 0) {
                    chosenPrefix = prefixCandidate1;
                } else {
                    const { data: list2, error: listErr2 } = await supabase.storage.from("workorder").list(prefixCandidate2);
                    if (listErr2) {
                        console.debug("list prefix2 error:", listErr2);
                    }
                    if (Array.isArray(list2) && list2.length > 0) {
                        chosenPrefix = prefixCandidate2;
                    }
                }
            } catch (e) {
                console.debug("Error while detecting storage prefix:", e);
            }
            ...
            // Try to find tanda1/tanda_tangan1 for jagarti and tanda2 for customer
            try {
            ...
            // Fallback to DB fields if storage lookup didn't yield URLs
                ...
                <div className="md:col-span-2 space-y-6">
                    <div className="bg-white rounded-lg shadow-lg p-6">
                        <div className="flex items-start justify-between">
                            <div>
                                <h1 className="text-3xl font-bold text-indigo-600 mb-1">{order.no_spk}</h1>
                        ...
                        <div className="bg-white rounded-lg shadow p-6">
                            ...
                            {photos.length > 1 && (
                                <div className="flex gap-2 mt-4 overflow-x-auto">
                                    {photos.map((p, idx) => (
                                        <button key={p.idx} onClick={() => animateCarousel(idx)} className={`flex-shrink-0 w-20 h-12 rounded overflow-hidden border ${idx === currentPhotoIdx ? 'ring-2 ring-indigo-400' : ''}`}>
